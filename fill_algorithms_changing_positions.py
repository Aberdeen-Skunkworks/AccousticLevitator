...
postions_grid = np.zeros((10,10))
x_gridpoints = np.linspace(-0.05  ,   0.05 , 10)
y_gridpoints = np.linspace(-0.05  ,   0.05 , 10)


for x_directon in range(len(x_gridpoints)):
    for y_directon in range(len(y_gridpoints)):
        ...
        print("At location: ", [x_directon , y_directon])
        postions_grid[x_directon,y_directon] = max_for_this_loop
    


## To export heatmap to vti file    
"""
...
for y in range(dims[1]):
    for x in range(dims[0]):
        imageData.SetScalarComponentFromDouble(x, y,0, 0, postions_grid[x,y])
